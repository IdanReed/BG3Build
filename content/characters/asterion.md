...
- name: The Fist in the Dark
  ...
  - 'STRUCTURALLY UNAVAILABLE, not an oversight — Karlach''s Soul Coins. The published "broken build" version of Open Hand Monk pairs it with Karlach, whose Infernal Machine lets her consume Soul Coins for bonus Fire damage and extra movement, and several guides treat that pairing as part of the package. This party has no Karlach, and no other character can use a Soul Coin at all, so that damage layer is simply unreachable here. Nothing to fix and nothing to route — recorded so the gap is not mistaken for a missing item. Asterion''s replacement damage layer is the Psionic Overload + Resonance Stone psychic package plus Gloves of Soul Catching in Act 3.'
  illithid:
