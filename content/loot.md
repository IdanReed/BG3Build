---
loot_guide:
- act: 1
  areas:
  - area: Nautiloid (Prologue)
    items: []
    events:
    - name: Recruit Lae'zel
      note: Freed from her pod on the ship.
    - name: Tadpole / Illithid Powers unlock
      note: You're infected here; the Dream Guardian later unlocks the Illithid Powers tree.
  - area: Ravaged Beach / Dank Crypt & Camp
    items:
    - name: Silver Pendant
      for: Shared utility swap
      core: true
      note: 'On a skeleton at the Harper outpost southwest of the Grove (X: 152 Y: 366). Grants Guidance at will. MANDATORY PICKUP: with Gale on the Fire Sorlock build there is no Cleric level anywhere in the party, so this pendant is the ONLY source of Guidance for the whole run. Grab it before the Grove and keep it as a shared exploration swap. Do not feed it to Gale''s orb.'
    - name: Deathstalker Mantle
      wiki: The Deathstalker Mantle
      for: Asterion
      core: true
      note: DARK URGE EXCLUSIVE — given by Sceleritas Fel at camp in Act 1. Once/turn, killing a creature turns you Invisible for 2 turns. Charles (the Dark Urge) receives it, then hands it to Asterion (the assassin's reposition/re-ambush cloak).
    events:
    - name: Withers — recruit + RESPEC
      note: Sarcophagus in the Dank Crypt (Overgrown Ruins). Enables respec (100g) / revive / hirelings at camp — needed for every respec in this plan.
    - name: Recruit Shadowheart, Astarion, Gale
      note: Beach/cliffs area.
  - area: Emerald Grove
    items:
    - name: Ring of Protection
      for: Asterion
      core: true
      note: 'Reward from Mol for completing Steal the Sacred Idol (after resolving the Grove; it is not pickpocketed from her). +1 AC and +1 to all saves — give this to the exposed unarmoured Monk.'
    - name: Corellon's Grace
      for: Asterion
      core: true
      note: 'Sold by Auntie Ethel in the Grove. EARLY LEVELS 2–4 ONLY: it improves Flurry/bonus-action punches by +1 and grants +2 saving throws while unarmoured. Unequip it when Tavern Brawler arrives at character level 5; ordinary main-action attacks otherwise swing the staff instead of receiving Tavern Brawler and unarmed riders.'
    - name: Safeguard Shield
      for: any
      core: false
      note: 'Sold by Dammon. +2 AC and +1 to all saving throws. NO LONGER GALE''S — the Fire Sorlock takes Dual Wielder at character level 4 and fills both hands with staves, so he cannot hold a shield. This is the main defensive regression of the build switch; the shield is free for anyone who can use it.'
    - name: Broodmother's Revenge
      for: Bonbon
      core: true
      note: 'After saving the Grove and convincing Kagha to stand down, find her isolated, knock her out non-lethally, and loot the amulet. Any healing coats Titanstring for +1d6 Poison per projectile for 2 turns; use a potion even at full HP before a Flourish nova. Skip it against poison immunity.'
    - name: Whispering Promise + Hellrider's Pride
      wiki:
      - The Whispering Promise
      - Hellrider's Pride
      for: any
      core: false
      note: 'Whispering Promise (Volo) + Hellrider''s Pride (Zevlor/Kagha): a Bless-on-heal + resistance-on-heal combo for a support healer.'
    - name: Gloves of Power
      for: any
      core: false
      note: Carried by Za'krug at the Grove gate (or looted from the goblin who wears it). +1 Sleight of Hand, and while the wearer bears the Absolute's Brand its weapon hits inflict Absolute's Bane (−1d4 to the target's attacks & saves). NOT a +attack/DC item — a cheap flat pickpocket helper.
    - name: Sorrow (glaive)
      wiki: Sorrow
      for: any
      core: false
      note: Emerald Grove Hidden Vault (Wolf altar; unlocked via the Enclave Library puzzle). +1 reach glaive that grants the Sorrowful Lash cantrip — a decent early reach weapon for an optional martial.
    events:
    - name: ⚠ Strange Ox — DO NOT KILL IT HERE
      note: 'The wandering Strange Ox in the Grove carries the Hat of Fire Acuity ONLY from Act 2 onward — kill it here and you get just the Shapeshifter''s Boon Ring, and Gale''s entire build loses the item it depends on. Leave it alone; kill it at Dammon''s blacksmith in Last Light Inn in Act 2. If you miss it there, it reappears in Rivington on a hill west of the requisitioned barn in Act 3.'
    - name: Save the Grove
      note: Central Act 1 hub — good path (kill goblin leaders) vs evil (raid with Minthara).
    - name: Recruit Wyll
      note: Training tieflings at the grove.
  - area: Sunlit Wetlands (Auntie Ethel)
    items:
    - name: The Sparkle Hands
      for: Asterion
      core: true
      note: 'Chest beneath the giant tree in the Decrepit Sanctuary. Offensive Bracers alternative: unarmed hits generate Lightning Charges, and charges grant advantage against metal-armoured enemies and constructs.'
    - name: Auntie Ethel's Hair (Hag's Hair)
      wiki: Auntie Ethel's Hair
      for: any
      core: true
      note: 'Vanilla provides one permanent +1 to one ability, uncapped. This modded run supplies one Hair per party member: Charles, Bonbon, and Gale take CHA 17→18; Asterion takes DEX 17→18.'
    events:
    - name: Get the Hag's Hair
      note: 'Fight Ethel in her lair, bring her below 20% HP, and let her take a turn to offer the bargain. Accepting the deal grants the Hair and lets her escape; killing her does NOT give the Hair. Letting her go can break non-Devotion Paladin oaths, which is acceptable for Charles''s planned Oathbreaker path.'
    - name: Rescue Mayrina
      note: Bitter Divorce questline.
  - area: Blighted Village & Whispering Depths
    items:
    - name: Haste Helm
      for: Charles
      core: true
      note: 'Moss-Covered Chest in the Blighted Village. Charles keeps its opening Momentum because the Ring of Arcane Synergy supplies his damage rider without consuming the head slot. Asterion can borrow it when Charles does not need the approach speed. ⚠ Gale no longer has Storm Sorcery''s Tempestuous Magic flight, and does not get Draconic Fly until character level 12 — but his head slot is committed to Shadespell and then the Hat of Fire Acuity, so he still cannot use this.'
    - name: Bracers of Defence
      for: Asterion
      core: true
      note: Apothecary's secret cellar beneath the village. +2 AC while wearing no armour and no shield — Asterion's AC without armour.
    - name: Sussur weapons (Dagger/Greatsword)
      wiki:
      - Sussur Dagger
      - Sussur Greatsword
      for: any
      core: false
      note: Craft at the Blacksmith with Sussur Tree Bark (from the Sussur Tree in the Whispering Depths). On-hit Silence — shuts down enemy casters.
    events:
    - name: The Necromancy of Thay
      note: Permanent Speak with Dead (risky WIS saves) — behind a locked gate, opened with the Dark Amethyst.
    - name: Whispering Depths / Sussur Tree
      note: Silence-bark for the weapons + Dark Amethyst; Phase Spider Matriarch; a pit drops to the Underdark.
  - area: Goblin Camp
    items:
    - name: Amulet of Misty Step
      for: Charles
      core: true
      note: 'Gilded chest in Priestess Gut''s chambers. Misty Step once per short rest solves Charles''s approach and elevation problems. Asterion already has Monk movement, the Sentient Amulet, and later the Night Walkers.'
    - name: Crusher's Ring
      for: Asterion
      core: true
      note: 'Take it from Crusher through dialogue or loot. +3m movement speed stacks with Longstrider and is Asterion''s selected second combat ring beside Ring of Protection.'
    - name: Gloves of Archery
      for: Bonbon
      core: true
      note: 'Sold by Grat. The proficiency is redundant, but +2 damage applies to every Titanstring or hand-crossbow hit — Bonbon''s selected early glove.'
    - name: Bow of Awareness
      for: Gale
      core: true
      note: 'Sold by Roah Moonglow in the Shattered Sanctum. Gale carries it in his otherwise-unused ranged slot solely for +1 Initiative, helping him establish Twinned Haste or Wet before the melee turns.'
    - name: Dual Hand Crossbows +1 — two sets
      for: Charles + Asterion
      core: true
      note: 'Farm four copies across Dammon, Roah Moonglow, Derryth Bonecloak, and A''jak''nir Jeera. Charles and Asterion each equip a pair in their ranged slots for main-hand and bonus-action fallback shots; Gale uses Bow of Awareness and Bonbon uses Titanstring.'
    - name: Arrows of Darkness — farmed supply
      for: Charles
      core: true
      note: 'Check arrow vendors after long rests and level-ups. A shot creates a concentration-free 3m Darkness cloud for 3 turns: preferably Bonbon places it before Charles switches to Phalar and activates Shriek, so Devil''s Sight grants Charles advantage for two-handed Phalar/GWM attacks while his concentration remains available for Bless, Divine Favour, or Hex.'
    - name: Boots of Striding
      for: Charles
      core: true
      note: 'Looted from Minthara. In Act 1, start Bless, Divine Favour, or Hex before entering the concentration-free Darkness Arrow cloud; after the Resonance respec, Charles can instead concentrate on his own Darkness. Starting concentration grants Momentum, and while concentrating he cannot be knocked Prone or moved against his will.'
    - name: Gloves of the Growling Underdog
      for: Charles
      core: false
      note: 'Dror Ragzlin''s treasure room. Advantage on melee weapon attacks when at least two enemies are within 3m of the target. Useful in Act 1 when conserving Darkness Arrows; later redundant when Risky Ring or attacking from Darkness already supplies advantage.'
    - name: Spidersilk Armour
      for: Gale
      core: false
      note: 'Looted from Minthara. Light armour with advantage on Constitution saves — an early concentration bridge before War Caster, after which the advantage is redundant.'
    - name: Goblin-leader loot (Gut / Ragzlin / Minthara)
      wiki: false
      for: any
      core: false
      note: The three goblin bosses drop assorted +1 weapons, potions, and gold — sweep the Shattered Sanctum before it turns hostile.
    events:
    - name: Volo's Ersatz Eye — permanent See Invisibility
      note: Rescue Volo (caged), let him operate at camp → Ersatz Eye (9m See Invisibility). Must be received via dialogue.
    - name: Recruit Halsin / Minthara
      note: Free Halsin (good) or side with goblins for Minthara (evil, raids the Grove).
  - area: Zhentarim Hideout (Waukeen's Rest / Risen Road)
    items:
    - name: The Spellsparkler
      for: Gale
      core: true
      note: Reward from Counsellor Florrick for rescuing her at the burning Waukeen's Rest. Quarterstaff — casting builds Lightning Charges. Gale's early staff.
    - name: Gloves of Thievery
      for: Asterion
      core: true
      note: Sold by Brem in the Zhentarim Basement (after 'Find the Missing Shipment'). Advantage on Sleight of Hand — core thief glove.
    - name: Titanstring Bow
      for: Bonbon
      core: true
      note: 'Brem (Zhentarim Basement). Bonbon''s selected Act-1 ranged weapon: equip the Hill Giant club main hand + Knife of the Undermountain King off-hand in the melee set, drink Bloodlust when adds can trigger it, and use a Hill Giant elixir for a single boss.'
    - name: Zhentarim vendor stock (Brem / Sparkle)
      wiki: false
      for: any
      core: false
      note: The basement traders sell Smokepowder Bombs/Barrels (chasm & burst strats), scrolls, and misc gear — a one-stop Act 1 shop. Steal-friendly.
    events:
    - name: Rescue the Grand Duke (Waukeen's Rest fire)
      note: Save Florrick → The Spellsparkler + Flaming Fist plotline.
    - name: Recruit Karlach
      note: On the Risen Road; Anders' paladins want her dead — help her vs kill for the reward.
  - area: Underdark
    items:
    - name: Boots of Stormy Clamour
      for: Gale
      core: true
      note: Sold by Omeluum (Ebonlake Grotto) after 'Help Omeluum Investigate the Parasite'. Applies 2 turns of Reverberation when you inflict a condition — a Reverberation staple, acquired in Act 1.
    - name: Pearl of Power Amulet
      for: Gale
      core: true
      note: 'Sold by Omeluum after Help Omeluum Investigate the Parasite. Restores one spell slot of level 3 or below per long rest — an extra Haste, Lightning Bolt, or Counterspell for Gale.'
    - name: Melf's First Staff
      for: Gale
      core: true
      note: 'Sold by Blurg. +1 Spell Save DC and +1 spell attacks. NOT a swap any more — with Dual Wielder at character level 4, Gale holds this OFF-HAND alongside Spellsparkler permanently, and the +1 spell attack applies to every individual Scorching Ray ray.'
    - name: The Shadespell Circlet + The Lifebringer
      for: Gale
      core: true
      note: 'Omeluum sells Shadespell after his quest: +1 Spell Save DC while obscured, Gale''s selected head. Blurg sells Lifebringer: 3 temporary HP when gaining Lightning Charges, a defensive alternative.'
    - name: Luminous Armour
      for: Charles
      core: true
      note: 'Selûnite Outpost trapped chest behind a hidden door. AC 15 + up to 2 DEX; Charles''s Divine Smite Radiant damage emits Radiating Shockwaves that penalise nearby enemy attacks.'
    - name: Phalar Aluve
      for: Charles
      core: true
      note: 'Longsword in stone near the Selûnite Outpost. ACT 1 through most of ACT 2: bind and wield it in both hands, take GWM at character level 6, and use Darkness Arrows for advantage. LATE ACT 2 after the Resonance Stone: respec GWM to Dual Wielder, summon/bind 3d8 Shadow Blade in the main hand, and move Phalar off-hand. Pre-cast Shriek when possible and keep the sword equipped so its 6m aura covers Charles and Asterion.'
    - name: Caustic Band + Club of Hill Giant Strength
      wiki:
      - Caustic Band
      - Club of Hill Giant Strength
      for: Bonbon
      core: true
      note: 'Caustic Band (Derryth) adds +2 Acid to every weapon hit, scaling with Bonbon''s Flourish projectiles. Break the Arcane Tower stool for the Club, then equip the Light club main hand with the Knife off-hand while Titanstring remains in the ranged slot; STR 19 preserves the elixir slot for Bloodlust.'
    - name: Elixirs of Bloodlust
      for: Bonbon
      core: true
      note: 'Farm/craft from Worg Fangs and check Cyrel, Derryth, and Stonemason Kith. In non-Honour mode, a kill grants an extra Action that benefits from Extra Attack. Use Hill Giant Strength instead for a single boss with no dependable kill.'
    - name: Ring of Mind-Shielding
      for: any
      core: false
      note: Omeluum's reward for 'Help Omeluum Investigate the Parasite' (Ebonlake Grotto — persuade, pay, or steal). Advantage on saves vs Charmed + can't be possessed/read — solid utility ring.
    events:
    - name: Help Omeluum
      note: Myconid Colony — reward includes the Amulet of Misty Step or a ring, opens Omeluum's shop (Boots of Stormy Clamour).
    - name: Arcane Tower
      note: Reactivate the tower; loot the Club of Hill Giant Strength + scrolls.
  - area: Grymforge
    items:
    - name: Disintegrating Night Walkers
      for: Asterion
      core: true
      note: Loot from True Soul Nere. Bonus-action Misty Step (1/short rest) + immunity to Web/Entangle/grease-slip — assassin mobility.
    - name: Adamantine armour pair (crafted)
      for: Bonbon + Charles
      core: true
      note: 'Use the two Mithral ores for Adamantine Splint Armour (Bonbon''s defensive alternative to Protecty Sparkswall) and Adamantine Scale Mail (Charles''s defensive alternative to Luminous Armour). Charles does not use a shield: he two-hands Phalar in Act 1, then uses Shadow Blade + Phalar Aluve after the Act-2 respec.'
    - name: The Protecty Sparkswall
      for: Bonbon
      core: true
      note: 'Trapped bridge chest in Grymforge. Bonbon''s selected late-Act-1 chest: +1 Spell Save DC improves Hold Person, Hypnotic Pattern, Fear, Slow, and Glyph of Warding. Adamantine Splint remains the defensive alternative.'
    - name: Wondrous Gloves
      for: Bonbon
      core: false
      note: 'Carried by a Grymforge Mimic near the Harper cache. +1 AC and one extra Bardic Inspiration provide another Slashing Flourish; use when the extra nova resource is worth losing Gloves of Dexterity accuracy.'
    - name: Armour of Uninhibited Kushigo
      for: Asterion
      core: false
      note: 'Reward from Sergeant Thrinn for returning her boots. Act 1 defensive alternative: Patient Defence enables a reaction unarmed counterattack when an enemy misses. Graceful Cloth remains selected.'
    - name: Sentient Amulet
      for: Asterion
      core: true
      note: Grymforge, locked Adamantine chest near the Lava Elemental. Ki Restoration restores 2 Ki once per long rest; this is the Monk's best Act 1 neck. Shatter is incidental and the rare version can cause Hysterical on a failed WIS save.
    events:
    - name: Adamantine Forge
      note: Pour Mithral + mould, then fight Grym (lure onto the forge). One-time crafting.
    - name: True Soul Nere
      note: Free the deep gnomes vs side with the duergar; loot Nere for the Night Walkers.
  - area: Mountain Pass / Rosymorn Monastery / Crèche Y'llek
    items:
    - name: Graceful Cloth
      wiki: The Graceful Cloth
      for: Asterion
      core: true
      note: Sold by Lady Esther (Rosymorn trail). +2 DEX (cap 20) + Cat's Grace (advantage on DEX checks — helps stealing). Asterion's all-game chest.
    - name: Gloves of Cinder and Sizzle
      for: Asterion
      core: true
      note: 'Sold by Lady Esther. Offensive Bracers alternative: +1d4 Fire on every unarmed hit plus a level-3 Scorching Ray once per long rest. Prefer Sparkle Hands against metal targets or fire resistance.'
    - name: Knife of the Undermountain King
      for: Bonbon
      core: true
      note: 'Sold by A''jak''nir Jeera. Equip it in Bonbon''s melee off-hand beside the Light Hill Giant club. Organ Rearranger lowers the critical threshold for Titanstring and spell attacks; its low-die damage reroll applies only to melee weapon attacks, not Bonbon''s ranged shots.'
    - name: Gloves of Dexterity
      for: Bonbon
      core: true
      note: A'jak'nir Jeera (Crèche). Sets DEX to 18 — lets the Bard dump DEX and pump CHA.
    - name: Gloves of Baneful Striking
      for: Charles
      core: true
      note: 'Sold by Lady Esther. Charles''s selected late-Act-1 glove: after a weapon hit, the target suffers −1d4 to saving throws for 2 turns, helping Asterion''s Stun and the casters'' control.'
    - name: Diadem of Arcane Synergy
      for: Bonbon
      core: true
      note: 'Ardent Jhe''rezath, Inquisitor''s Chamber (Crèche). After Bonbon inflicts Hold Person, Frightened, or another spell condition, Arcane Synergy adds CHA to each subsequent ranged weapon attack for 2 turns. Her multi-hit Titanstring Flourishes exploit the flat rider; replace it with the Helmet of Arcane Acuity in Act 2.'
    - name: Ring of Arcane Synergy
      for: Charles
      core: true
      note: 'Worn by Gish Far''aag in Crèche Y''llek. Damaging an enemy with Booming Blade grants Arcane Synergy for 2 turns, adding Charles''s CHA modifier to subsequent bound-weapon attacks: two-handed Phalar in Act 1 and Shadow Blade after the Resonance respec. The Ring and Diadem grant the same non-stacking condition, so the Diadem goes to Bonbon.'
    - name: Strange Conduit Ring
      for: Charles
      core: true
      note: 'Crèche Y''llek. While Concentrating, weapon attacks deal +1d4 Psychic. In Act 1, concentrate on Bless, Divine Favour, or Hex while a Darkness Arrow supplies obscurement; after the Stone respec, self-cast Darkness. Resonance Stone vulnerability and critical hits multiply the Psychic dice.'
    - name: Gloves of Belligerent Skies
      for: Gale
      core: true
      note: 'Inquisitor''s Chamber. Thunder/Lightning/Radiant applies Reverberation, including from save-based Lightning Bolt, but the passive triggers once per attack: an AoE normally affects only its first logged target. Spellmight Gloves remain poor for Gale''s save-based core.'
    - name: Necklace of Elemental Augmentation
      for: Gale
      core: false
      note: 'Inquisitor''s Chamber display case. Adds CHA to native elemental cantrip damage such as Shocking Grasp or Ray of Frost; equip after spending Pearl of Power or during low-resource encounters.'
    - name: Periapt of Wound Closure
      for: any
      core: false
      note: Lady Esther. Auto-stabilise + always heal max — strong survivability amulet.
    - name: A'jak'nir Jeera — githyanki vendor stock
      wiki: false
      for: any
      core: false
      note: The Crèche quartermaster sells githyanki martial weapons, medium armour, whetstones, and the Gloves of Dexterity / Knife of the Undermountain King (core picks) — clear her stock (buy or steal) before leaving the Crèche, which locks.
    - name: Rosymorn / Crèche side-loot
      wiki: false
      for: any
      core: false
      note: 'The monastery and Crèche hold misc boots, potions, githyanki gear, and the Dawnmaster''s Crest puzzle path to the Blood of Lathander — don''t leave it under-looted. (Note: ''Everlasting Vigour'' +2 STR is Araj''s Act 2 potion, not a Rosymorn item.)'
    events:
    - name: Blood of Lathander (legendary mace)
      wiki: The Blood of Lathander
      note: Rosymorn puzzle (Dawnmaster's Crest). +3 mace with Sunbeam + a party self-revive-at-0-HP aura — don't miss.
    - name: Zaith'isk → 'Awakened' permanent buff
      note: 'Crèche infirmary machine: pass 3 saves (DC 12/15/18) for permanent Awakened (Illithid powers as bonus actions); failing = permanent −2 stat.'
- act: 2
  areas:
  - area: Shadow-Cursed Lands
    items:
    - name: Moonlantern
      for: any
      core: false
      note: Required to survive the region (shadow-curse protection). From Kar'niss after the Harper ambush, or Isobel/Balthazar.
    - name: Shadow of Menzoberranzan
      for: any
      core: false
      note: Head slot, dropped by Kar'niss (the drider carrying the Moonlantern). Cast Invisibility 1/short rest while in shadow/darkness — excellent for any stealth or scout character (a strong alternative escape/ambush tool alongside Asterion's kit).
    - name: Ring of Mental Inhibition
      for: Gale
      core: true
      note: 'In a locked chest in the House in Deep Shadows, just east of the Shadowed Battlefield waypoint. When a foe fails a saving throw against Gale''s spells they gain Mental Fatigue for 2 turns, stacking the odds for the next Command — and applying a condition also triggers Boots of Stormy Clamour. ⚠ Per the wiki it does NOT trigger on the saves a creature makes to shake off an existing effect, so it will not extend Hold Person. Swap it in over Coruscation for control-focused fights.'
    events:
    - name: Free the pixie (Dolly Dolly Dolly)
      note: Freeing the pixie in the Moonlantern grants the party permanent Pixie's Blessing (curse immunity without holding the lantern). Missable.
  - area: Last Light Inn
    items:
    - name: Shadow Blade Ring
      wiki: Shadow Blade Ring
      for: Charles
      core: true
      note: Arabella's reward for 'Find Arabella's Parents' (delivered at camp). Bonus-action Shadow Blade, short-rest recharge, no concentration — Charles's backup blade if his class-cast blade is unavailable. Asterion is an unarmed Monk and does not use it.
    - name: Coruscation Ring
      for: Gale
      core: true
      note: 'Trapped chest in a hidden cellar room. THE ILLUMINATION ENGINE: it applies Radiating Orb when the WEARER is illuminated — the target does not need to be lit. Radiating Orb then makes the target Illuminated, which switches on Callous Glow''s +2 radiant, which procs Gloves of Belligerent Skies'' Reverberation. Since only GALE needs light, this never conflicts with Charles standing in a Darkness cloud. Keep Gale lit with the Light cantrip early, then Daylight (Enchant Item) on Bonbon''s weapon from character level 8.'
    - name: Covert Cowl
      wiki: Covert Cowl
      for: Charles
      core: false
      note: Cellar, on a Meenlock. While Obscured, crit threshold −1 — Charles's Act-2 Darkness head after Bonbon replaces the Diadem with the Helmet of Arcane Acuity. The freed Diadem remains his raw-damage alternative once Risky Ring displaces his Act-1 Arcane Synergy Ring.
    - name: Hat of Fire Acuity
      for: Gale
      core: true
      note: '⚠⚠ THE build-defining item — carried by the Strange Ox at Dammon''s blacksmith. Kill it HERE (not in the Act 1 Grove, where it does not yet carry the hat). Dealing Fire damage grants 2 turns of Arcane Acuity: +1 spell attack AND +1 spell save DC per remaining turn, capped at 10, losing 1 per turn and 2 per hit taken. Each Scorching Ray ray deals Fire damage separately, so one level-4 cast (5 rays) takes Gale from 0 to the cap. Missed it? The Ox reappears in Rivington in Act 3.'
    - name: Evasive Shoes
      for: Gale
      core: false
      note: Sold by Mattis. Damage-focused alternative to Boots of Stormy Clamour for fights where the Reverberation engine is not the plan.
    - name: Potent Robe
      for: Gale
      core: false
      note: 'Alfira hands it over at Last Light in ACT 2 — ONLY if she survived Act 1 (as the Dark Urge, Charles must knock her out before camp, not kill her). DEMOTED by the Fire Sorlock switch: it adds CHA to CANTRIP damage, and this build stops casting cantrips in combat after character level 3. Keep it as a generic robe until Armour of Landfall in Act 3.'
    events:
    - name: Protect Isobel — DON'T let the inn fall
      note: Stop the assassin Marcus from taking Isobel; if she dies the moonshield drops and the inn (vendors + quests) is lost. Missable.
    - name: Dammon (Karlach)
      note: Second-stage infernal-engine upgrade + hellish gear.
  - area: Moonrise Towers
    items:
    - name: Risky Ring
      for: Charles
      core: true
      note: Sold by Araj Oblodra. Advantage on all attack rolls (disadvantage on saves) — Charles's always-advantage crit enabler (re-sold in Act 3 if missed).
    - name: Ne'er Misser
      for: Bonbon
      core: true
      note: Sold by Roah Moonglow at Moonrise. Pair this force-damage hand crossbow with Hellfire Hand Crossbow to produce the multi-hit Arcane Acuity setup as soon as Act 2.
    - name: Spineshudder Amulet
      for: Gale
      core: true
      note: 'In a Mimic in Isobel''s bedroom (upper floor, Moonrise). Applies Reverberation on ranged SPELL-ATTACK hits only (not weapon hits or save spells). PROMOTED TO CORE by the Fire Sorlock switch — it was near-worthless for the old save-based Storm nukes and is excellent now that Gale''s main spell is 3–7 spell attack rolls per cast. With Boots of Stormy Clamour it reliably stacks Reverberation to Prone.'
    - name: Moonrise Towers vendor & prison loot
      wiki: false
      for: any
      core: false
      note: Quartermaster Talli and the Moonrise traders sell heavy armour, martial weapons, and scrolls; the prison/temple levels hide gear and story items. Clear it before the assault — Moonrise is a point of no return.
    events:
    - name: Araj Oblodra — permanent +2 STR
      note: If Astarion bites her on request, she gives a Potion of Everlasting Vigour = PERMANENT +2 Strength (any PC can drink). Astarion disapproves.
  - area: Gauntlet of Shar
    items:
    - name: Killer's Sweetheart
      for: Charles
      core: true
      note: Self-Same Trial (kill your shadow clone). After you KILL, your next attack is a GUARANTEED crit (refreshes on long rest) — a free crit-smite for Charles. ⚠ As an Oathbreaker, do NOT kill the clone with Control Undead (it instakills the copy but the ring won't drop) — land a normal killing blow.
    - name: Callous Glow Ring
      for: Gale
      core: true
      note: 'Opulent chest in the vault room near Balthazar. +2 Radiant damage against ILLUMINATED creatures — applied per ray, so up to +14 on a level-6 Scorching Ray, and the radiant damage also procs Gloves of Belligerent Skies. The Coruscation Ring is what makes targets Illuminated in the first place, so wear the pair together. ⚠ Take it off against Shar worshippers and Justiciars.'
    - name: Hellfire Hand Crossbow
      for: Bonbon
      core: true
      note: Dropped by Yurgir. Pair with Ne'er Misser for Bonbon's Act 2 dual-hand-crossbow Arcane Acuity engine; Asterion is an unarmed Monk.
    - name: Dark Justiciar Half-Plate & Spear of Night
      wiki:
      - Dark Justiciar Half-Plate
      - Spear of Night
      for: any
      core: false
      note: Silent Library hidden chamber (tied to Shadowheart's arc) — solid heavy-armour + spear.
    - name: Full Dark Justiciar set (Mask / Gauntlets / Helmet)
      wiki:
      - Dark Justiciar Mask
      - Dark Justiciar Gauntlets (Rare)
      - Dark Justiciar Helmet
      for: any
      core: false
      note: 'Justiciar pieces span Grymforge and the Gauntlet. The Dark Justiciar Helmet grants Covert Critical while Obscured, making it a second Darkness head option for Charles or another weapon attacker. Asterion''s unarmed Monk wants Mask of Soul Perception later.'
    - name: Umbral Gems (progression) + trial loot
      wiki: false
      for: any
      core: false
      note: Each trial hands an Umbral Gem (needed to reach the Nightsong) plus incidental gear — don't skip a trial thinking it's optional loot.
    events:
    - name: The Three Trials + Umbral Gems
      wiki: false
      note: Soft-Step / Self-Same / Faith-Leap each give a gem (a 4th from Yurgir) → opens the way to the Nightsong.
    - name: Nightsong / Dame Aylin choice
      note: Free Dame Aylin (she becomes a camp ally + helps kill Ketheric) vs kill her for Shar/Ketheric — pivotal, tied to Shadowheart.
    - name: Mirror of Loss here is CRACKED
      note: The Gauntlet's Mirror is non-functional — the working +2-ability Mirror is in ACT 3 (Cloister of Sombre Embrace).
  - area: Mind Flayer Colony
    items:
    - name: Resonance Stone
      for: Asterion
      core: true
      note: 'Near the Mind-Archive Interface (Necrotic Laboratory), late in Act 2. This pickup triggers Charles''s respec—usually at character level 9—to Warlock 5 / Paladin 4, replacing GWM with Dual Wielder and the two-handed Phalar setup with 3d8 Shadow Blade main hand + Phalar off-hand; Savage Attacker is the Paladin feat. Asterion carries the 9m aura to double his Manifestation/Psionic Overload and Charles''s Shadow Blade/Strange Conduit damage; holster it against psychic damage or dangerous mental-save effects.'
    - name: Staff of Cherished Necromancy
      for: any
      core: false
      note: Dropped by Balthazar — on the good path he dies in the Gauntlet of Shar (Act 2), so it usually drops THERE, not the Colony. Free-casts a stored Necromancy spell on a kill + necro-save disadvantage — niche (no necromancer in this party).
    events:
    - name: Apostle of Myrkul
      note: Ketheric's final form — freeing the Nightsong earlier gives crucial aid.
    - name: Astral-Touched Tadpole / the Emperor
      note: Consuming it grants strong Illithid powers — a permanent character change / one-time choice.
  - area: Reithwin Town — Mason's Guild
    items:
    - name: Helmet of Arcane Acuity
      for: Bonbon
      core: true
      note: ACT 2 (Mason's Guild is in Reithwin Town, not the Act 3 Lower City). Locked/trapped Gilded Chest in the secret basement. Weapon hits stack Arcane Acuity (+spell attack/save DC) — the Bard's control engine.
    - name: Reithwin Town side-loot
      wiki: false
      for: any
      core: false
      note: 'Reithwin is dense with loot: the Tower of the Sun/Waning Moon (light-lever puzzle), the apothecary and Thorm mansion, Gerringothe Thorm''s gold hoard, and the Mason''s Guild vault scrolls. Sweep the whole town while the moonlantern/pixie keeps the curse off.'
    events:
    - name: Thorm Mausoleum puzzle
      note: Press the family plaques in mural order to open the way to the Gauntlet of Shar.
    - name: Oathbreaker Knight (Charles)
      note: 'Keep Charles''s temporary Vengeance oath intact through Act 1. At the Resonance Stone, respec first, then break the fresh oath to enter Oathbreaker cleanly. If he broke the oath early, pay the Oathbreaker Knight to restore it before Withers will allow the respec.'
  - area: House of Healing (Reithwin) — MISSABLE
    items:
    - name: Eversight Ring
      wiki: Eversight Ring
      for: Asterion
      core: true
      note: 'ACT 2 — Reithwin''s House of Healing, locked opulent chest in the morgue lab. Immunity to Blinded lets Asterion fight beside Charles inside Darkness without losing accuracy. Swap it in for Darkness-heavy encounters. ⚠ MISSABLE: grab it BEFORE defeating Ketheric / clearing the Mind Flayer Colony, or the Shadow-Cursed Lands lock forever.'
    - name: House of Healing alchemy & surgeon loot
      wiki: false
      for: any
      core: false
      note: The morgue/operating theatre is stuffed with alchemy ingredients, potions, and gear; the Malus Thorm 'Surgeon' fight drops scalpels and healing supplies.
    events:
    - name: Malus Thorm (the Surgeon)
      note: Optional fight in the House of Healing — the nurses and Malus guard the morgue where the Eversight Ring is stored.
- act: 3
  areas:
  - area: Rivington
    items:
    - name: Hellrider's Longbow
      wiki: Hellrider Longbow
      for: Bonbon (contested with Gale)
      core: true
      note: 'Sold by Ferg Drogher in Rivington. +initiative. NEWLY CONTESTED: the Fire Sorlock guide says this should always go to the Sorlock on fire parties, and Gale has no feat left for Alert. Bonbon keeps it by default because she must land the first weapon hit to open the Band of the Mystic Scoundrel loop; Gale keeps Bow of Awareness and drinks an Elixir of Vigilance for fights where he must go first.'
    - name: Strange Ox (second chance)
      wiki: false
      for: Gale
      core: false
      note: 'If the Hat of Fire Acuity was missed at Last Light in Act 2, the Strange Ox reappears here on a hill west of the requisitioned barn. This is the last chance at the item Gale''s build depends on.'
    - name: Nyrulna
      for: any
      core: false
      note: Reward from the Circus of the Last Days quest. Legendary trident that returns when thrown and bursts for Thunder AoE — best-in-slot for any throwing build.
    - name: Band of the Mystic Scoundrel
      for: Bonbon
      core: true
      note: 'Won from AKABI''s wheel game at the Circus of the Last Days (Rivington): hit the jackpot → you''re teleported (ALONE) to a Chult jungle where it sits in a backpack (Nyrulna is in the same jungle). Cast Enchantment/Illusion as a BONUS action after a weapon hit — the Bard''s flourish→Command loop. ⚠ NOT at Sorcerous Sundries; don''t over-spin the wheel (≈16 spins closes it).'
    - name: Spellmight Gloves
      for: Gale
      core: true
      note: 'Lucretious''s reward for Find Dribbles the Clown at the Circus (also pickpocketable). −5 to spell attack rolls for +1d8 damage. PROMOTED TO CORE by the Fire Sorlock switch — previously rejected because Storm''s nukes were save-based, and now excellent because Scorching Ray is an attack roll firing 3–7 times. MANAGE THEM: first cast of a fight with the gloves OFF to build Arcane Acuity, then switch them ON once Acuity covers the −5.'
    - name: Rivington vendors & Requisitioned Barn
      wiki: false
      for: any
      core: false
      note: Ferg Drogher, the Circus traders, and the Open Hand temple hold early Act 3 gear, arrows, and scrolls; the barn has a hidden cellar. Good first-stop shopping/stealing before the Lower City.
    events:
    - name: Circus of the Last Days
      note: Side content + vendors; a couple of unique items and story hooks.
  - area: Sorcerous Sundries / Ramazith's Tower
    items:
    - name: Markoheshkir
      for: Gale
      core: true
      note: 'Ramazith''s Tower, inside a Globe of Invulnerability (See Invisibility to spot the lever, then DC 20 Arcana to disable the globe). +1 spell attack and DC, plus Arcane Battery for one free spell of any level. Attune Kereska''s Favour to FLAME OF WRATH (not Bolts of Doom, which was the old Storm pick): Fire resistance, +proficiency bonus to Fire spell damage applied per ray, and Heat generation. ⚠ Heat deals unavoidable self-damage each turn and threatens Twinned Haste — do not attune Flame of Wrath until Armour of Landfall is equipped.'
    - name: Armour of Landfall
      for: Gale
      core: true
      note: 'Sold by Lorroakan''s Projection or Rolan on the SUNDRIES GROUND FLOOR (1700g). AC 13 + DEX light armour, +1 Spell Save DC, and — the real reason — ADVANTAGE ON CONSTITUTION SAVING THROWS. That advantage replaces the Halfling Luck and the War Caster feat this build cannot have, and it is what makes Markoheshkir''s Heat safe to carry while concentrating on Twinned Haste. Gale''s Human Civil Militia already grants light-armour proficiency, so the Warlock dip is not needed to wear it.'
    - name: Stolen scrolls (Chain Lightning, Freezing Sphere, Cone of Cold)
      wiki:
      - Scroll of Chain Lightning
      - Scroll of Otiluke's Freezing Sphere
      - Scroll of Cone of Cold
      for: Bonbon / Gale
      core: false
      note: Buy or steal these as situational ammunition for the party's actual casters. Asterion is a Monk/Thief and no longer has Magical Ambush.
    - name: Quickspell Gloves
      for: any
      core: false
      note: Sold at Sorcerous Sundries (Rolan). Cast a cantrip as a BONUS action, 1/short rest — handy QoL for any caster (Gale/Bonbon).
    - name: Robe of the Weave
      for: any
      core: false
      note: Ramazith's Tower, inside a Globe of Invulnerability (needs See Invisibility + Arcana DC20). +2 AC, +1 Spell Save DC & Spell Attack, and heal 1d6 whenever you succeed on a save vs a spell — strong caster robe for anyone not using the Potent Robe.
    events:
    - name: Lorroakan vs Nightsong choice
      note: At Ramazith's Tower, choose whether to protect Dame Aylin or side with Lorroakan. This choice does not award Gontr Mael; that bow drops from the Steel Watcher Titan in the Foundry.
  - area: Cloister of Sombre Embrace (House of Grief)
    items: []
    events:
    - name: Mirror of Loss — permanent +2 ability (the REAL one)
      note: The working Mirror. Pass Religion DC20 / Arcana DC25 (or read the note) to understand it, then offer a prayer to Shar (DC25) to land a permanent +2 (cap 24). This is the +2 the whole plan assumes — every PC should visit.
  - area: Stormshore Tabernacle
    items:
    - name: Amulet of the Devout
      for: any
      core: false
      note: 'Main offering chest in the basement. +2 spell save DC + an extra Channel Divinity. NO LONGER GALE''S — the Fire Sorlock has no Cleric level and therefore no Channel Divinity to recharge, and his amulet slot belongs to Spineshudder. Still a fine +2 DC neck for anyone else. ⚠️ Looting it inflicts the Castigated By Divinity curse UNLESS Jaheira loots it wearing Khalid''s Gift.'
    events:
    - name: The curse trap
      note: Don't brick the amulet — use Jaheira + Khalid's Gift to loot the offering chest cleanly.
  - area: Temple of Bhaal / Murder Tribunal
    items:
    - name: Sarevok's Horned Helmet
      for: Charles
      core: true
      note: From Sarevok at the Murder Tribunal. Lowers crit threshold + on-kill bonuses — crit-fish core.
    - name: Bloodthirst
      for: optional weapon attacker
      core: false
      note: Legendary +2 dagger looted from Orin. Its crit and hand-specific passives support weapon and dual-wield builds, but Charles keeps Phalar Aluve equipped to sustain Shriek beside Shadow Blade.
    - name: Crimson Mischief
      for: any
      core: false
      note: Also carried by Orin. Its hand-specific bonuses support dual wielding, but Charles keeps Phalar Aluve beside Shadow Blade to sustain Shriek.
    - name: Ring of Murderous Opportunity + Sword of Chaos
      wiki:
      - Ring of Murderous Opportunity
      - Sword of Chaos
      for: any
      core: false
      note: Ring of Murderous Opportunity (Orin) — bonus damage + extra reaction attacks, strong for an assassin. Sword of Chaos (Sarevok, alongside the Horned Helmet) — +2 wounding longsword.
    events:
    - name: Murder questline (Dark Urge)
      note: Charles IS the Dark Urge — this arc and its embrace-or-resist choice are his story climax.
  - area: House of Hope (Raphael)
    items:
    - name: Amulet of Greater Health
      for: Bonbon
      core: true
      note: On a pedestal in the Archive. Sets CON to 23 + advantage on CON saves. REALLOCATED to the Bard (was Asterion) to armour Hold Monster concentration — the melee auto-crit engine — with no neck conflict; stacks with the Bard's War Caster feat. (Gale keeps the Amulet of the Devout.)
    - name: Helldusk Gloves
      for: Charles
      core: true
      note: Raphael's gloves add fire damage to weapon hits and improve spell attacks/DC across Charles's Shadow Blade attacks.
    - name: Helldusk Armour
      for: Charles
      core: false
      note: Excellent self-proficient defensive alternative when Charles needs raw AC and resistances more than Luminous Armour's Radiating Shockwaves.
    - name: Orphic Hammer + Infernal Rapier
      wiki:
      - Orphic Hammer
      - Infernal Rapier
      for: any
      core: false
      note: Orphic Hammer frees Orpheus (story); the Infernal Rapier is a strong CHA-scaling weapon option.
    - name: Helldusk Boots
      for: any
      core: false
      note: Fire immunity plus a bonus-action reposition and anti-prone protection. Use as defensive flex boots; Charles does not need to wear the full Helldusk set.
    - name: House of Hope vault (Korrilla / Hope's reward)
      wiki: false
      for: any
      core: false
      note: Raphael's boudoir and the archive hold a big gold/gem hoard, Elixirs of Universal Resistance, and scrolls; Hope hands over gear if freed. Loot the whole house during the fight.
    events:
    - name: Raphael fight
      note: Defeat Raphael (or loot during the Hope rescue) for the House of Hope gear.
  - area: Cazador's Palace
    items:
    - name: Rhapsody
      for: Gale
      core: true
      note: 'Carried by Cazador Szarr. PROMOTED TO CORE by the Fire Sorlock switch: Scarlet Remittance stacks +1 attack roll, +1 damage, AND +1 spell save DC per kill (max 3), and this build is the rare one that uses all three — the damage applying per Scorching Ray ray. Gale wields it off-hand behind Markoheshkir, which requires Dual Wielder. ⚠ Per the wiki, since Patch 5 it only builds stacks on killing living hostile targets. Asterion''s unarmed Monk still cannot use it.'
    - name: Cazador's palace loot (ritual chamber & kennels)
      wiki: false
      for: any
      core: false
      note: The palace holds a gold hoard, potions of Vampirism, the Ritual Dagger, and the trapped ritual-chamber gear. Sweep it fully during Astarion's questline — it doesn't reopen after the finale.
    events:
    - name: Astarion's Ascension choice
      note: 'MAJOR: ascend Astarion (power boost, darker) vs stay a Spawn (keeps Vampire weaknesses but the ''good'' arc). Affects Asterion directly.'
  - area: Steel Watch Foundry
    items:
    - name: Gontr Mael
      for: Bonbon
      core: true
      note: Looted from the Steel Watcher Titan in the Control Centre. The legendary longbow grants Celestial Haste and Guiding Bolt, but no initiative; compare it with Hellrider's Longbow when Bonbon must act first.
    events:
    - name: Disable the Steel Watch
      note: Destroy the Foundry to weaken the endgame assault; loot the Steel Watcher Titan before leaving.
  - area: Lower City (Zhentarim / vendors)
    items:
    - name: Ne'er Misser
      for: Bonbon
      core: false
      note: Sold by Roah Moonglow, first at Moonrise Towers in Act 2 and again here if missed. Pair with Hellfire Hand Crossbow for Bonbon's multi-hit Arcane Acuity setup; its force damage bypasses many resistances.
    - name: Birthright
      for: Bonbon
      core: true
      note: 'Sold at Sorcerous Sundries by Rolan / Lorroakan''s projection (ground floor). +2 Charisma helm (cap 22). REASSIGNED — Gale''s head slot now belongs permanently to the Hat of Fire Acuity, without which his whole build stops working, so Birthright goes to Bonbon instead.'
    - name: Cloak of the Weave
      for: Gale
      core: true
      note: 'Sold by Helsik at the Devil''s Fee, but ONLY once her special stock is unlocked through dialogue. +1 Spell Save DC and +1 spell attack rolls — Gale''s best-in-slot cloak, and the spell attack applies per ray. ⚠ Per the wiki its Absorb Elements ability is bugged and non-functional; buy it for the flat +1/+1.'
    - name: Helm of Balduran + Wyrmway loot (Ansur)
      wiki:
      - Helm of Balduran
      for: any
      core: false
      note: The Wyrmway (accessed from the Counting House basement) leads to the Ansur dragon fight → Helm of Balduran (regen + crit immunity), one of the best defensive helms in the game. The trials en route also hand out gear.
    - name: Facemaker's Boutique + Devil's Fee
      wiki: false
      for: any
      core: false
      note: Facemaker's (Figaro) sells headwear/clothing incl. hats with cast-a-spell effects; the Devil's Fee (Helsik) sells rare rings, scrolls, and Netherese/ritual gear. Prime stealing targets — see Asterion's Act 3 pickpocket setup.
    events:
    - name: Iron Throne (time-sensitive)
      note: Rescue prisoners (Duke Ravengard, gnomes) before it floods — a real timer.
---
